...
const MobileNavbar = ({ closeMenuBar , onGetQuoteClick }) => {
  const [openSubMenuIndex, setOpenSubMenuIndex] = useState(null);
  const [openSubMenuDrodownIndex, setOpenSubMenuDrodownIndex] = useState({
    parent: null,
    sub: null,
  });
  ...
  return (
    <div className="md:hidden block">
      {navMenuData.map((menu, index) => (
        ...
        >
          <span
            onClick={(e) => menu.subNavBar && e.preventDefault()}
            className="px-2 flex justify-between items-center gap-2 hover:bg-sky-700 duration-150 ease-in-out hover:text-white text-[#575757] py-[8px] rounded-[10px]"
          >
            <NavLink to={menu.url || "#"}>{menu.label}</NavLink>
            
            {menu.subNavBar && (
              <i
                className={`fa-solid fa-chevron-down text-xs mt-[2px] transform transition-transform duration-300 ${
                  ...
                  >
                    <span className="px-2 flex items-center justify-between">
                      ...
                      {subMenu.dropdown && (
                        <i
                          className={`fa-solid fa-chevron-down text-xs mt-[2px] transform transition-transform duration-300 ${
                            openSubMenuDrodownIndex.parent === index &&
                            openSubMenuDrodownIndex.sub === subIndex
                              ? "rotate-180"
                              : ""
                          }`}
                        ></i>
                      ...
                      <ul
                        className={`${
                          ...
                          openSubMenuDrodownIndex.sub === subIndex
                            ? "block"
                            : "hidden"
                        } showme bg-white rounded-md !px-0 py-2`}
                      >
